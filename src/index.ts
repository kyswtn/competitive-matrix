...
function updateSearchParams(
	...
	let search = "";
	if (newIds.length > 0)
		search += `${idParamName}=${newIds.join(`&${idParamName}=`)}`;
...
function renderApps(sdks: Sdk[], apps: App[]) {
	...
	h3.textContent =
		from.id === to.id
			? `Apps using ${from.name}`
			: `Apps moved to ${to.name} from ${from.name}`;
	section.append(h3);
...
window.addEventListener("load", async () => {
	...
	const selectedSdks = ids
		.map((id) => allSdks.find((sdk) => sdk.id === id))
		.filter((id) => !!id);
